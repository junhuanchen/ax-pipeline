...
void fbdev_flush(lv_disp_drv_t * drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    // printf("x1 %d, x2 %d, y1 %d, y2 %d, areax %d, areay %d, lenth %d\n", act_x1, act_x2, act_y1, act_y2, area->x2, area->y2, finfo.line_length);
    ...
    if(vinfo.bits_per_pixel == 32 || vinfo.bits_per_pixel == 24) 
    {
        ...
        // int32_t y;
        char file[] = "fb.rgb";
        // uint32_t *rotated = NULL;
        // rotated = (uint32_t *)malloc(sizeof(uint32_t) * vinfo.xres * vinfo.yres);
        ...
        //     color_p += w;
        // }
        int WidthIn = area->x2 + 1;
        int HeightIn = area->y2 + 1;
        ...
        for (int i = 0; i < HeightOut; i ++)
        {
            ...
            {
                memcpy(&fbp32[(i*WidthOut + j)], &color_p[((HeightIn - j - 1)*WidthIn + i)], 4);
            }          
        }
        
                
    
        // memcpy(fbp32, (uint32_t *)rotated, vinfo.xres * vinfo.yres * 4);


        // FILE* fp = fopen(file, "wb+");
        // fwrite(rotated, 1, vinfo.xres * vinfo.yres * 4, fp);
        // fclose(fp);
    }
    /*16 bit per pixel*/
